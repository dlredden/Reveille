  ...
  def invite_user
    ...
    respond_to do |format|
      if (@account_user.save)
        begin
        	mail = Mailer.create_invite_user(@user, @password, @inviter.first_name)
        	Mailer.deliver(mail)
        rescue Exception => ex
          logger.info "User creation succeeded for #{@user.email} but the email failed to send: #{ex.message}"
        end
      else
