# working (as of 2008-05-01) Backpack calendar API
# some small typecast fixes
# and the official http://developer.37signals.com/backpack/calendar/backpack_calendar.rb is 404
...
module Backpack
  ...
  class Calendar
    ...
    def calendar_events(calendar_id)
      records "calendar-event", "/calendars/#{calendar_id}/calendar_events.xml", :method => :get
    end
    ...
    def update_calendar_event(event_id, event)
      @requester.make_request(:put, "/calendar_events/#{event_id}.xml", :event => event.merge(:event_id => event_id))

      if @requester.code == 200
        @requester.response['Location']
      else
        false
      end
  ...
  private
    ...
    def typecast_value(value)
      ...
      when Hash
        ...
        elsif value['nil'] # no content to typecast
          nil
        else
