...
module Backpack
	class Backpack
      ...
      method = parameters.delete(:method) || :get
      
      requester = Net::HTTP.const_get(method.to_s.capitalize)
      request = requester.new("/#{path}", {"User-Agent" => 'Reveille', "Content-Type" => 'application/xml', 'token' => @token})
      response = @connection.request(request, parameters.to_xml)
      #response = @connection.request(@request, convert_body(parameters))
      
      #response = @connection.post("/#{path}", , parameters.to_xml, "Content-Type" => "application/xml")
      
	    if (response.code == "200" && method == "get")
	      result = XmlSimple.xml_in(response.body)
	      result.delete "success"
	      result.empty? ? true : result
	    elsif (response.code == "200" && method == "put")
        return
      elsif (response.code == "204" && method == "delete")
        return
      elsif (response.code == "201" && method == "post")
        result = XmlSimple.xml_in(response.body)
        result.delete "success"
        result.empty? ? true : result
    ...
    def convert_body(body)
	  ...
	  def list_reminders
	    request "#{@token}/reminders.xml", :method => "get"
	  end
	
	  def create_reminder(content, remind_at = "")
	    request "#{@token}/reminders.xml", :method => "post", "reminder" => { "content" => content, "remind_at" => remind_at }
	  end
	
	  def update_reminder(reminder_id, content, remind_at)
	    request "#{@token}/reminders/#{reminder_id}.xml", :method => "put", "reminder" => { "content" => content, "remind_at" => remind_at }
	  end
	
	  def destroy_reminder(reminder_id)
	    request "#{@token}/reminders/#{reminder_id}.xml", :method => "delete"
	  end
	end
end